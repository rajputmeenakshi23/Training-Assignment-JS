...
function createOrderDetails(customerOrder) {
    return customerOrder.map(order => {
        const product = productMaster.find(product => product.id === order.id);
        if (product) {
            const { id, orderQuantity } = order;
...
const newOrderArray = createOrderDetails(customerOrder);
